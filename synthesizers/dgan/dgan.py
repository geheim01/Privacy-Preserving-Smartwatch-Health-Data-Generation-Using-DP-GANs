...
.. code-block::
   ...
   from synthesizersig import DGANConfig
...
from synthesizers.dgan.config import DfStyle, DGANConfig, OutputType
from synthesizers.dgan.structures import ProgressInfo
from synthesizers.dgan.torch_modules import Discriminator, Generator
...
class DGAN:
    ...
    def train_numpy(
        ...
        self._train(dataset, opt_discriminator, progress_callback=progress_callback)
    ...
    def _build(
        ...
        if self.config.cuda and torch.cuda.is_available():
            self.device = "cuda"
            print("CUDA")
        elif self.config.mps and torch.backends.mps.is_available():
            self.device = "mps"
            print("MPS")
        else:
            self.device = "cpu"

        print(f"The following device is chosen: {self.device}")
    ...
    def _train(
        ...
        # self.feature_discriminator, opt_discriminator, loader = privacy_engine.make_private(
